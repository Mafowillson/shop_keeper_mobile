...
class _CustomerDebtsScreenState extends State<CustomerDebtsScreen> {
  ...
  List<Customer> _filtered(List<Customer> all) {
    var list = all;
    if (_selectedFilter == 'With Debt') list = list.where((c) => c.totalDebt > 0).toList();
    if (_selectedFilter == 'High') list = list.where((c) => c.totalDebt > 30000).toList();
    if (_selectedFilter == 'Medium') list = list.where((c) => c.totalDebt > 10000 && c.totalDebt <= 30000).toList();
    if (_selectedFilter == 'Low') list = list.where((c) => c.totalDebt > 0 && c.totalDebt <= 10000).toList();
    if (_selectedFilter == 'Clear') list = list.where((c) => c.totalDebt == 0).toList();
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Add Customer', style: AppTextStyles.headingL),
                    ...
                    : RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (_, i) => _CustomerCard(
                            customer: filtered[i],
                            onTap: () => context.push('/owner/debts/${filtered[i].id}'),
                          ),
                        ),
                      ),
              ),
...
class _SummaryCard extends StatelessWidget {
  ...
  const _SummaryCard({required this.totalDebt, required this.debtorCount, required this.totalCount});
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text('$debtorCount debtor${debtorCount == 1 ? '' : 's'} out of $totalCount customer${totalCount == 1 ? '' : 's'}',
                style: AppTextStyles.bodyM.copyWith(color: Colors.white70)),
          ],
        ),
      );
